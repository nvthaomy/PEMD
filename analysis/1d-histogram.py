...
#hist = hist/A  #gives density per length
binmidAll = 0.5*(bins[1:]+bins[0:-1])

data = np.vstack([binmidAll,histAll]).T
np.savetxt('{}hist_all.dat'.format(['x','y','z'][ax]),data,header='bin-midpt\tHistogram')
...
if anames:
    ...
    #get number density
    density = histMasked/histAll
    data = np.vstack([binmidMasked,density,histMasked]).T
    np.savetxt('{}hist_{}.dat'.format(['x','y','z'][ax],'_'.join(anames)),data,header='bin-midpt\tNumberDensity\tCount\tMasked atoms: {}'.format(args.atoms))
    ...
    fig,axs = plt.subplots(nrows=1, ncols=1, figsize=[3,2])
    axs.plot(binmidAll, histAll, marker=None,ls='-',lw=0.75,mfc="None",ms=2)
    #axs.legend(loc='best',prop={'size': 5})
    plt.xlabel('{}'.format(['x','y','z'][ax]))
    plt.ylabel('Count')
